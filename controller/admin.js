...
// API endpoint for hawala transactions
router.get('/api/hawala-transactions', async (req, res) => {
    try {
        const transactions = await HawalaTransaction.getHawalaTransactions();
        res.json(transactions || []);
    } catch (error) {
        console.error('Error fetching hawala transactions:', error);
        res.status(500).json({ error: error.message || 'Error fetching hawala transactions' });
    }
});
...
router.get('/api/bank-transactions', async (req, res) => {
    try {
        ...
        console.error('Error fetching bank transactions:', error);
        res.status(500).json({ error: error.message || 'Error fetching bank transactions' });
    }
});
